...
def reservation(request):
    if request.user.groups.filter(name='Manager').exists():
        if request.method == 'GET':
            # return all reservatoins that are today and in the future
            bookings = Booking.objects.filter(reservation_date__gte=datetime.today().date())
            serialized_bookings = ReservationSerializer(bookings, many=True)
            return Response(serialized_bookings.data, status=status.HTTP_200_OK)
        else:
            message = {'message': 'request method is not allowed'}
            return Response(message, status=status.HTTP_405_METHOD_NOT_ALLOWED)
    else:
        message = {'message': 'Your not authorized for this action'}
        return Response(message, status=status.HTTP_401_UNAUTHORIZED)

@api_view(['DELETE'])
@permission_classes([IsAuthenticated])
def reservationItem(request, pk=None):
    if request.user.groups.filter(name='Manager').exists():
        if request.method == 'DELETE':
            booking = get_object_or_404(Booking, pk=pk)
            booking.delete()
            message = {'message': 'Reservation ID {} deleted successfully'.format(pk)}
            return Response(message, status=status.HTTP_200_OK)
        else:
            message = {'message': 'request method is not allowed'}
            return Response(message, status=status.HTTP_405_METHOD_NOT_ALLOWED)
